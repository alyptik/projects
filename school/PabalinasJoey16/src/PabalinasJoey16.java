 ...
 * a .csv file into a LinkedList<StarHouses>
 * object, then appropriately
 * sets the data members of each node
 * and prints the results. It then
 * instantiates a new LinkedList<StarHouses>
 * object, and initializes each node with
 * the mutated fields of the first list,
 * finally printing the the now mutated
 * list of StarHouse objects.
 *
 * @author Pabalinas, Joey
 * ICS 211 Assignment #16
 * 10/19/17
 *
 */
...
public class PabalinasJoey16 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			LinkedList<StarHouses> mutated = new LinkedList<>();
			...
			for (int i = 1; i <= houses.getSize(); i++) {
				...
				mutated.add(new StarHouses());

				// uppercase whole name
				mutated.get(i).updateName(
					houses.get(i).
						updateName("").
						toUpperCase());
				// uppercase first two letters
				mutated.get(i).updateStar(
					houses.get(i).
						updateStar("").
						substring(0,2).
						toUpperCase() +
					houses.get(i).updateStar("").substring(3));
				// obfuscate with bitwise operations and mod 360
				mutated.get(i).updateDegree((int)(((
					houses.get(i).
						updateDegree(0) ^
						((System.nanoTime()) % 360 << 3) &
						0xdeadbeef) >> 2) % 360));
			}

			// print mutated fields
			System.out.println("");
			for (int i = 1; i <= mutated.getSize(); i++)
				System.out.printf("%d\t%s\n", i, mutated.get(i).getFields());
	...
	private static LinkedList<StarHouses> readFile(String inputFile) {
		...
		if (scanFile != null) {
			...
			for (int i = 0; i < size; i++) {
				...
				// set fields
				house.updateName(names[i]);
				house.updateStar(stars[i]);
				house.updateDegree(degrees[i]);
				houseList.add(house);
			}
...
class StarHouses {
	 ...
	 */
	String updateName(String inputName)
	{
		String old = name;
		// don't change value if empty string given
		name = (inputName.matches("")) ? old : inputName;
		return old;
	}

	/**
	 * The setter/getter for star
	 * @param inputStar the star
	 * @return the original star
	 */
	String updateStar(String inputStar)
	{
		String old = star;
		// don't change value if empty string given
		star = (inputStar.matches("")) ? old : inputStar;
		return old;
	}

	/**
	 * The setter/getter for degree
	 * @param inputDegree the degree
	 * @return the original degree
	 */
	int updateDegree(int inputDegree)
	{
		int old = degree;
		// don't change value if 0 given
		degree = (inputDegree == 0) ? old : inputDegree;
		return old;
	}
