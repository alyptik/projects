...
public class PabalinasJoey09 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			StarHouses[] mutated = new StarHouses[houses.length];

			// print fields
			for (int i = 0; i < houses.length; i++) {
				System.out.printf("%d\t%s\n", i, houses[i].getFields());

				// map mutations into a new array
				mutated[i] = new StarHouses();
				// uppercase whole name
				mutated[i].updateName(
					houses[i].
						updateName("").
						toUpperCase());
				// uppercase first two letters
				mutated[i].updateStar(
					houses[i].
						updateStar("").
						substring(0,2).
						toUpperCase() +
					houses[i].updateStar("").substring(3));
				// obfuscate with bitwise operations and mod 360
				mutated[i].updateDegree((int)(((
					houses[i].
						updateDegree(0) ^
	 ...
	 */
	private static StarHouses[] readFile(String inputFile) {
