...
static void complain( const char *message, const int messageParam, const int level )
{
	if( level < maxNestLevel )
		{
		if( !doPure )
...
static void dumpHex( FILE *inFile, long length, int level, const int isInteger )
{
...
static int oidToString( char *textOID, int *textOIDlength, const BYTE *oid, const int oidLength )
{
	BYTE uuidBuffer[ 32 ];
	long value;
...
static void displayString( FILE *inFile, long length, int level, const STR_OPTION strOption )
{
	char timeStr[ 64 ];
	long noBytes = length;
	int lineLength = 48, i;
...
static void printASN1object( FILE *inFile, ASN1_ITEM *item, int level )
{
	...
	case BITSTRING:
		...
		if( (unsigned)item->length <= sizeof( int ) )
			{
	...
	case OID:
		...
		if( ( oidInfo = getOIDinfo( buffer, ( int ) item->length ) ) != NULL )
			...
			if( ( ( doPure ) ? 0 : INDENT_SIZE ) + ( level * 2 ) + 18 + \
				strlen( oidInfo->description ) + 2 + length >= (unsigned)outputWidth )
				{
				printString( level, "%c", '\n' );
				if( !doPure )
					printString( level, "%s", INDENT_STRING );
...
static int printAsn1( FILE *inFile, const int level, long length, const int isIndefinite )
{
	ASN1_ITEM item;
	long lastPos = fPos;
